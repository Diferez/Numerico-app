...
Es = {'title':"Análisis numérico",'correr':'Correr', 'biseccion':"Bisección", 'busquedas':"Búsquedas Incrementales", 'raicesI':'raices','gaussSimple':'Gaussiana Simple','solucion':'Solucion', 'graficadora':'Graficadora',
          ...
          'dFmultroot' : '- Una de las condiciones para garantizar la convergencia del método de Newton es que f ´ (Xv) tiene que ser diferente de cero. Si al ejecutar el método de Newton se observa que f´(xn) se acerca a cero, la velocidad del método disminuye y existe una posible raíz múltiple.',
          ...
          'dFfixepo' : '- Se utiliza principalmente para determinar las raíces de una función de la forma f (x) = 0, si se cumplen las condiciones de convergencia. Para este caso no es necesario tener un intervalo, su objetivo principal es encontrar la raíz de una función basada en un valor inicial, una tolerancia y varias iteraciones.',
          ...
          'dFjaco' : '- El método de Jacobi siempre converge si la matriz A es una diagonal estrictamente dominante y puede converger incluso si no se cumple esta condición.\nPara verificar la convergencia del método, se calcula el radio espectral (ρ)',
          'dFsaidel' : '- Se evalúan las siguientes condiciones: La matriz es estrictamente dominante diagonalmente por filas, De la siguiente identidad: A = D - L - U, El máximo de los valores absolutos de las raíces de la ecuación característica de la matriz es menor que 1',
          'dFlagran' : '- Este método reduce el problema restringido con n variables a uno sin restricciones de n + k variables, donde k es igual al número de restricciones y cuyas ecuaciones se pueden resolver más fácilmente.',
          ...
          'dFgausto' : '- Utiliza la eliminación gaussiana, pero tiene una diferencia en el procedimiento utilizado en el intercambio de filas y columnas, de modo que el elemento pivote es el valor máximo de cada submatriz obtenido en las operaciones de cada escenario.',
          'dFlus' : '- la matriz triangular inferior L colocando los multiplicadores en los lugares indicados por sus índices y en los números diagonales principales 1. La matriz U se obtiene de la matriz resultante del proceso de extracción al eliminar la columna que corresponde a los términos independientes.',
          ...
          'dFcubi' :'- Esta función consiste en una unión de polinomios públicos, uno para cada intervalo. La idea central es que, en lugar de usar un solo polinomio para interpolar todos los datos, puede usar segmentos de polinomios entre pares de datos de coordenadas',
...
En = {'title':"Numerical analysis",'correr':'Run', 'biseccion':"Bisection", 'busquedas':"Incremental search", 'raicesI':'roots', 'gaussSimple':'Simple Gaussian','solucion':'Solution','graficadora':'Plotter',
          ...
          'dFincremental':'- Care should be taken when choosing the increase (Delta) for which the function is to be evaluated, if the increase is very small, there is a risk that the process is very expensive, and if it is very large, there is a risk of not detect the root\n',
          'dFnewton':'- Select an initial value close enough to the searched root',
          'dFmultroot': '- One of the conditions to guarantee the convergence of Newton´s method is that f ´ (Xv) has to be different from zero. If when executing Newton´s method it is observed that f´(xn) approaches zero, the speed of the method decreases and there is a possible multiple root.',
          ...
          'dFfixepo' : '- It is mainly used to determine roots of a function of the form f (x) = 0, if convergence conditions are met. For this case it is not necessary to have an interval, its main objective is to find the root of a function based on an initial value, a tolerance and a number of iterations.',
          ...
          'dFjaco' : '- Jacobi´s method always converges if matrix A is strictly dominant diagonal and can converge even if this condition is not satisfied.\nTo verify the convergence of the method the spectral radius (ρ) is calculated',
          'dFsaidel' : '- The following conditions are evaluated: The matrix is ​​strictly dominant diagonally by rows, Of the following identity: A = D - L - U, The maximum of the absolute values ​​of the roots of the characteristic equation of the matrix is ​​less than 1',
          'dFlagran' : '-This method reduces the restricted problem with n variables to one without restrictions of n + k variables, where k is equal to the number of restrictions, and whose equations can be solved more easily.',
          ...
          'dFgausto' : '- It uses Gaussian elimination, but it has a difference in the procedure used in the exchange of rows and columns, so that the pivot element is the maximum value of each submatrix obtained in the operations of each scenario.',
          'dFlus' : '- The lower triangular matrix L placing the multipliers in the places indicated by their indexes and in the main diagonal numbers 1. The matrix U is obtained from the matrix resulting from the extraction process by eliminating the column corresponding to the independent terms.',
          ...
          'dFcubi' :'- This function consists of a union of public polynomials, one for each interval. The central idea is that, instead of using a single polynomial to interpolate all data, you can use polynomial segments between pairs of coordinate data',
